...
echo "IP Address, MAC Address, Vendor, Hostname, OS, Open Ports, Protocols, Services" > "$OUTPUT_FILE"

# Perform an advanced scan
sudo nmap -O -sV -p- --script=default "$IP_RANGE" -oG - | awk '
/Up$/{ip=$2}
...
/Ports:/{split($0, p, "Ports: "); split(p[2], ports, ", "); open_ports=""; protocols=""; services="";
    ...
    print ip "," (mac ? mac : "N/A") "," (vendor ? vendor : "Unknown") "," $5 "," $4 "," open_ports "," protocols "," services
}' >> "$OUTPUT_FILE"
